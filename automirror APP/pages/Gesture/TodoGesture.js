        ...
        axios.get(`${deviceAddress}/api/gesture?message=todo-capture`)
                      .then((response) => {
                        ...
                        if (response.status === 200) {
                          console.log("일정 제스처 촬영");
                          Alert.alert("일정 제스처 촬영", "일정 제스처 촬영 시작");
                          navigation.navigate("TodoGestureCapture");
                        } else {
                          console.error("일정 제스처 촬영 오류:", response.status);
                          Alert.alert("일정 제스처 촬영", "일정 제스처 촬영 실패");
                        }
                      })
                      .catch((error) => {
                        ...
                        if (response.status === 200) {
                          ...
                          Alert.alert( 
                            '일정 제스처 확인', '제스처 확인이 끝나면 확인 종료를 눌러주세요.', [ 
                                  {text: '확인 종료', onPress: () => todoCheckExit()}, 
                            ]
                          );
                        } else {
                          console.error("일정 제스처 확인 오류:", response.status);
                          Alert.alert("일정 제스처 확인", "일정 제스처 확인 실패");
                        }
                      })
                      .catch((error) => {
                        console.error("네트워크 오류 발생:", error);
    ...
    const todoCheckExit = () => {
        axios.get(`${deviceAddress}/api/gesture?message=todo-check-exit`)
                      .then((response) => {
                        console.log(response.status);
                        if (response.status === 200) {
                          console.log("일정 제스처 확인 종료");
                          Alert.alert("일정 제스처 확인 종료", "일정 제스처 확인 종료");
                        } else {
                          console.error("일정 제스처 확인 종류 오류:", response.status); 
                          Alert.alert("일정 제스처 확인 종료", "일정 제스처 확인 종료 실패");
                        }
                      })
                      .catch((error) => {
                        console.error("네트워크 오류 발생:", error);
        ...
        <View style={styles.container}>
            ...
            <View style={styles.body}>
              <View stsyle={styles.top}>
                <Text style={styles.main}>일정 제스처 설정</Text>
                <Ionicons
                ...
                <View style={styles.top}>
                  <TouchableOpacity onPress={()=> navigation.navigate("TodoGestureCapture")
                  // todoCapture()
                  }>
                    <Text style={styles.text}>일정 제스처 촬영</Text>      
                  </TouchableOpacity>
                  <TouchableOpacity onPress={()=> todoCheck()}> 
                      <Text style={styles.text}>일정 제스처 확인</Text>      
                  </TouchableOpacity>
