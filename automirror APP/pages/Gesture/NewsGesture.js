...
export default function NewsGesture() {
        ...
        axios.get(`${deviceAddress}/api/gesture?message=news-capture`)
                      .then((response) => {
                        ...
                        if (response.status === 200) {
                          console.log("뉴스 제스처 촬영");
                          Alert.alert("뉴스 제스처 촬영", "뉴스 제스처 촬영 시작");
                          navigation.navigate("NewsGestureCapture");
                        } else {
                          console.error("뉴스 제스처 촬영 오류:", response.status);
                          Alert.alert("뉴스 제스처 촬영", "뉴스 제스처 촬영 실패");
                        }
                      })
                      .catch((error) => {
                        ...
                        if (response.status === 200) {
                          ...
                          Alert.alert( 
                            '뉴스 제스처 확인', '제스처 확인이 끝나면 확인 종료를 눌러주세요.', [ 
                                  {text: '확인 종료', onPress: () => newsCheckExit()}, 
                            ]
                          );
                        } else {
                          console.error("뉴스 제스처 확인 오류:", response.status);
                          Alert.alert("뉴스 제스처 확인", "뉴스 제스처 확인 실패");
                        }
                      })
                      .catch((error) => {
                        console.error("네트워크 오류 발생:", error);
    ...
    const newsCheckExit = () => {
        axios.get(`${deviceAddress}/api/gesture?message=news-check-exit`)
                      .then((response) => {
                        console.log(response.status);
                        if (response.status === 200) {
                          console.log("뉴스 제스처 확인 종료");
                          Alert.alert("뉴스 제스처 확인 종료", "뉴스 제스처 확인 종료");
                        } else {
                          console.error("뉴스 제스처 확인 종류 오류:", response.status); 
                          Alert.alert("뉴스 제스처 확인 종료", "뉴스 제스처 확인 종료 실패");
                        }
                      })
                      .catch((error) => {
                        console.error("네트워크 오류 발생:", error);
        ...
        <View style={styles.container}>
            ...
            <View style={styles.body}>
              <View stsyle={styles.top}>
                <Text style={styles.main}>뉴스 제스처 설정</Text>
                <Ionicons
                ...
                <View style={styles.top}>
                  ...
                  <TouchableOpacity onPress={()=> newsCheck()}> 
                      <Text style={styles.text}>뉴스 제스처 확인</Text>      
                  </TouchableOpacity>
